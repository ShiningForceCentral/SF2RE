 ...
 *
 *	It finds lea loc(pc), a0 instructions 
...
static main(void) {
	...
	pattern = "41 FA"; // opcode for lea loc(pc), a0
	...
	for(addr=FindBinary(ScreenEA(),7,pattern);addr!=BADADDR;addr=FindBinary(addr+1,7,pattern)){
		...
		if (opnd!=""){
			...
			if (action==-1) break;
			if (action==1){
				Message(form("\n0x%d : lea %s,a0 changed to lea %s(pc), a0",addr,opnd, opnd));
				SetManualInsn(addr,rep);
			}
			else{
				Message(form("\n0x%d : lea %s,a0 NOT changed to lea %s(pc),a0",addr,opnd, opnd));
			}
