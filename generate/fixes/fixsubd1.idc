 ...
 *	instructions :  
 *	sub.w   %s,d1 were displayed as subi.w  %s,d1. 
 *
...
static main(void) {
	...
	pattern = "04 41"; // opcode for sub.w  %s,d1
	...
	for(addr=FindBinary(ScreenEA(),7,pattern);addr!=BADADDR;addr=FindBinary(addr+1,7,pattern)){
		...
		if (opnd!=""){
			...
			rep = form("sub.w   %s,d1",opnd);
		
			Jump(addr);
		
			//action = AskYN(1,form("Change representation from subi.w  %s,d1 to sub.w   %s,d1 ?",opnd, opnd));
			action = 1;
		
			if (action==-1) break;
			if (action==1){
				Message(form("\n0x%d : subi.w  %s,d1 changed to sub.w   %s,d1",addr,opnd, opnd));
				SetManualInsn(addr,rep);
			}
			else{
				Message(form("\n0x%d : subi.w  %s,d1 NOT changed to sub.w   %s,d1",addr,opnd, opnd));
			}
