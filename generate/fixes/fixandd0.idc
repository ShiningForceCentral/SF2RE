 ...
 *	instructions :  
 *	and.w   %s,d0 were displayed as andi.w  %s,d0. 
 *
 *	It finds lea loc(pc), a1 instructions 
...
static main(void) {
	...
	pattern = "02 40"; // opcode for lea loc(pc), a1
	...
	for(addr=FindBinary(ScreenEA(),7,pattern);addr!=BADADDR;addr=FindBinary(addr+1,7,pattern)){
		...
		if (opnd!=""){
			...
			rep = form("and.w   %s,d0",opnd);
		
			Jump(addr);
		
			//action = AskYN(1,form("Change representation from andi.w  %s,d0 to and.w   %s,d0 ?",opnd, opnd));
			action = 1;
		
			if (action==-1) break;
			if (action==1){
				Message(form("\n0x%d : andi.w  %s,d0 changed to and.w   %s,d0",addr,opnd, opnd));
				SetManualInsn(addr,rep);
			}
			else{
				Message(form("\n0x%d : andi.w  %s,d0 NOT changed to and.w   %s,d0",addr,opnd, opnd));
			}
