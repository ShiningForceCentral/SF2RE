 ...
 *
 *	It finds lea loc(pc), a4 instructions 
...
static main(void) {
	...
	pattern = "49 FA"; // opcode for lea loc(pc), a4
	...
	for(addr=FindBinary(ScreenEA(),7,pattern);addr!=BADADDR;addr=FindBinary(addr+1,7,pattern)){
		...
		if (opnd!=""){
			...
			rep = form("lea     %s(pc), a4",opnd);
		
			Jump(addr);
		
			action = AskYN(1,form("Change representation from lea %s to lea %s(pc), a4 ?",opnd, opnd));
		
			if (action==-1) break;
			if (action==1){
				Message(form("\n0x%d : lea %s,a4 changed to lea %s(pc), a4",addr,opnd, opnd));
				SetManualInsn(addr,rep);
			}
			else{
				Message(form("\n0x%d : lea %s,a4 NOT changed to lea %s(pc),a4",addr,opnd, opnd));
			}
