 ...
 *	instructions :  
 *	sub.w   %s,d0 were displayed as subi.w  %s,d0. 
 *
...
static main(void) {
	...
	pattern = "04 40"; // opcode for sub.w   %s,d0
	...
	for(addr=FindBinary(ScreenEA(),7,pattern);addr!=BADADDR;addr=FindBinary(addr+1,7,pattern)){
		...
		if (opnd!=""){
			...
			rep = form("sub.w   %s,d0",opnd);
		
			Jump(addr);
		
			//action = AskYN(1,form("Change representation from subi.w  %s,d0 to sub.w   %s,d0 ?",opnd, opnd));
			action = 1;
		
			if (action==-1) break;
			if (action==1){
				Message(form("\n0x%d : subi.w  %s,d0 changed to sub.w   %s,d0",addr,opnd, opnd));
				SetManualInsn(addr,rep);
			}
			else{
				Message(form("\n0x%d : subi.w  %s,d0 NOT changed to sub.w   %s,d0",addr,opnd, opnd));
			}
