 ...
 *
 *	It finds jmp (loc).w instructions 
...
static main(void) {
	...
	pattern = "4E F8"; // opcode for jmp (loc).w
	...
	for(addr=FindBinary(ScreenEA(),7,pattern);addr!=BADADDR;addr=FindBinary(addr+1,7,pattern)){
		...
		if (opnd!=""){
			...
			if (action==-1) break;
			if (action==1){
				Message(form("\n0x%d : jmp %s changed to jmp (%s).w",addr,opnd, opnd));
				SetManualInsn(addr,rep);
			}
			else{
				Message(form("\n0x%d : jmp %s NOT changed to jmp (%s).w",addr,opnd, opnd));
			}
