 ...
 *
 *	It finds lea loc(pc), a7 instructions 
...
static main(void) {
	...
	pattern = "4F FA"; // opcode for lea loc(pc), a7
	...
	for(addr=FindBinary(ScreenEA(),7,pattern);addr!=BADADDR;addr=FindBinary(addr+1,7,pattern)){
		...
		if (opnd!=""){
			...
			rep = form("lea     %s(pc), a7",opnd);
		
			Jump(addr);
		
			action = AskYN(1,form("Change representation from lea %s to lea %s(pc), a7 ?",opnd, opnd));
		
			if (action==-1) break;
			if (action==1){
				Message(form("\n0x%d : lea %s,a7 changed to lea %s(pc), a7",addr,opnd, opnd));
				SetManualInsn(addr,rep);
			}
			else{
				Message(form("\n0x%d : lea %s,a7 NOT changed to lea %s(pc),a7",addr,opnd, opnd));
			}
